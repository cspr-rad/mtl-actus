...
** DONE show the eval of the automaton agreeing with the word.
/finished the code for this, have to write it up./
* Future work
** Compiling general timed finite automata from MTL specs
The algorithm to generate the automaton that corresponds to a given MTL spec is known, but we have not implemented it in the current work. /write more/
** Implementing the remaining ACTUS taxonomy
There is a folk wisdom among ACTUS circles that PAM is 80% of the work. However, it still remains for us to implement the other 30something contracts in the ACTUS taxonomy. /write more/
