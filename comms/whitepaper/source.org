...
#+END_SRC
With ~disjunct~ derived from ~negate~ and ~conjunct~, and the unary temporal operators given in the usual way as combinations of the binary temporal operators ~since~ and ~until~ with ~mt_t~ (the value true). In particular, ~eventually~ ($\Diamond \phi$) is given $T U \phi$ and ~always~ ($\Box \phi$) is given $\neg \Diamond \neg \phi$. We additionally have the temporal operator ~next~ ($\circ \phi$) from $(\neg T) U \phi$, saying that $\phi$ is in the subsequent timestamp. In this MTL, indexing an operator with the window ~(Timestamp.infinity, Timestamp.infinity)~ means a degenerate case where the temporal operator simplifies to a linear temporal logic (LTL) operator.
** Automata
In the context of Metric Temporal Logic (MTL), automata play a crucial role in both theoretical analysis and practical applications. Timed automata, in particular, are closely related to MTL as they provide a way to model and verify real-time systems with quantitative timing constraints.
